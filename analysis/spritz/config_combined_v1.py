...
EFT signal: 7 mll-binned DYSMEFTsim LO NanoAOD samples, 55 subsamples each
  sm, w1_{op}, wm1_{op} for 27 operators (morphing convention for combine)

Backgrounds: Fabian's DY MiNNLO, TT, VV, single top, GGToLL + data
  k-factor (MiNNLO / SM_MG) applied bin-by-bin at plotting stage, not here.

Deploy on LLR:
  mkdir -p /grid_mnt/data__data.polcms/cms/adufour/spritz_fabian/configs/combined_v1
  cp this file .../configs/combined_v1/config.py

  Then add DYSMEFTsim_LO_mll_* entries to Fabian's samples.json
  (see notes/spritz.md for xsecs and NanoAOD paths)
"""
...
colors["DYll"]       = "#f89c20"
colors["sm"]         = "#000000"
# EFT w1/wm1 colors: use cycling petroff (arbitrary, we use custom EFT plotter anyway)
