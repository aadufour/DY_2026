# ruff: noqa: E501
"""
config_propcorr_v1.py  —  propcorr_v1: v9 setup, EFT signal replaced by the
propagator-corrected DY SMEFTsim samples
=============================================================
Identical to config_v9.py (same backgrounds, data, regions, variables,
nuisances, binning) except the EFT signal datasets point at the
propagator-corrected production (DYSMEFTsim_LO_propcorr_mll_<bin>,
see notes/propagator_correction.md) instead of the baseline
(DYSMEFTsim_LO_mll_<bin>). Kept as a separate parallel config rather than
adding propcorr as extra subsamples in v9 so the two can be run/compared
independently.
...
# -- EFT signal (DY SMEFTsim LO propcorr, 7 mll bins, 55 subsamples each) --
# EFT: 406 → runner filters events to those with exactly 406 LHEReweightingWeights
# (same reweighting scheme as baseline v9 -- 27 operators x {w1,wm1} + sm -- only
# the underlying model/generation differs, so the weight count carries over unchanged)
for b in MLL_BINS:
