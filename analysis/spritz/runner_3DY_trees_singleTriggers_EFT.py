        ...
        for dataset_name in results:
            ...
            for region in regions:
                ...
                for cwgt in check_weights:
                    events[cwgt] = check_weights[cwgt]["func"](events, dataset_name) if not isData else events.weight # [767, 947, 1.3e+03, 1.01e+03, ..., 1.51e+03, 1.02e+03, 1.07e+03, 1.15e+03]
                    mask = regions[region]["mask"] & events[f"mask_{dataset_name}"]
                    if len(events[mask]) == 0:
                        continue
                    # print(f"--> {dataset_name} region {region} check weight {cwgt} integral {ak.sum(events[cwgt][mask])}")
                    for variable in variables:
                        if ("save_events" in variables[variable].keys()) and (variables[variable]["save_events"]): 
                            ...
                            if "weight" not in results[dataset_name]["events"][region].keys(): results[dataset_name]["events"][region]["weight"] = events[cwgt][mask]
