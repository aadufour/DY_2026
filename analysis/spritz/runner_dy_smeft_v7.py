"""
runner_dy_smeft_v7.py
=====================
v7 of the DY SMEFTsim runner. Changes from v6 (runner_dy_smeft.py):

1. Theory variations (PDF + QCD scale) are no longer locked to dataset == "Zjj".
   They are now controlled exclusively by special_analysis_cfg["do_theory_variations"].

2. The nom-only variation filter is conditional: when do_theory_variations=True,
   all registered variations (including theory systematics) are kept and fill
   separate slices of the "syst" histogram axis.
