# ruff: noqa: E501
"""
config_syst_full.py  —  Fabian's backgrounds + DY SMEFTsim LO EFT signal + ALL systematics
============================================================================================
Same as config_syst.py but with ALL experimental systematics enabled:
  - do_theory_variations = True  → QCDscale, PDFweight, alphaS, PSWeight from NanoAOD
  - do_variations = True         → mu_reco, mu_idiso, mu_trig, PU, prefireWeight, tt_ptrw
...
NOTE: histos.root must be produced with this config (do_variations=True etc. already set).
      The syst config already ran with all flags on — spritz-cards only needs to be re-run.
