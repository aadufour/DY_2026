...
assign_run_period = lambda events, *args, **kwargs: events
...
dnn_cfg = special_analysis_cfg.get("dnn", None)
onnx_session = ort.InferenceSession(dnn_cfg["model"], sess_opt) if dnn_cfg else None
dnn_t = dnn_transform(dnn_cfg["cumulative_signal"]) if dnn_cfg else None
