# ruff: noqa: E501
"""
config_dy_smeftsim_v8.py  —  DY SMEFTsim LO, morphing-convention names + theory systematics
==============================================================================================
Changes from v7 (config_dy_smeftsim_v7.py):

1. Uses runner_dy_smeft_v8.py which produces 3-axis histograms (variable,
   category, syst) instead of v7's 4-axis (variable, category, syst,
   check_weights). spritz-postproc expects exactly 3 axes, so the manual
   pkl preprocessing step required by v7 is no longer needed.

Everything else identical to v7:
- Morphing-convention subsample names: sm, w1_{op}, wm1_{op}
- do_theory_variations = True  →  PDF + QCD scale variation histograms
- nuisances: QCDscale (lheScaleWeight_envelope) + PDF (lhePdfWeight_square)
