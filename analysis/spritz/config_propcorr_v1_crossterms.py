# ruff: noqa: E501
"""
config_propcorr_v1_crossterms.py  —  propcorr_v1 + the 351 cross-term (w11)
EFT subsamples, needed for 2D/N-D operator fits with correct SM+Lin+Quad+Mixed
yields (previously the "double" fits implicitly assumed the mixed term = 0,
since no w11_<opi>_<opj> process existed anywhere in the pipeline).
=============================================================
Identical to config_propcorr_v1.py in every other respect (same backgrounds,
data, regions, variables, binning). See notes/combine.md, "2D (Double) EFT
Scans" chapter, for the full diagnosis of why the cross terms were missing
and how AnomalousCouplingMorphing_comb consumes them once present.

The underlying LHEReweightingWeight array already has all 406 configurations
per event (verified against the MG5 reweight card,
.../DYSMEFTMll50_120_propcorr_reweight_card.dat): index 0 = SM, 1..27 = w1,
28..54 = wm1, 55..405 = the 351 pairs in itertools.combinations(OPERATORS, 2)
order. config_propcorr_v1.py only ever extracted the first 55 - this config
extracts all 406.
...
Runner: runner.py (unchanged, same as v9)
"""
...
# Cross terms (LHEReweightingWeight indices 55..405): raw yield at c_i=+1,
# c_j=+1 simultaneously. Index = 55 + position in
# itertools.combinations(OPERATORS, 2), matching the exact ordering of the
# "<op_i>_<op_j>" launch blocks in the MG5 reweight card (verified against
# DYSMEFTMll50_120_propcorr_reweight_card.dat). AnomalousCouplingMorphing_comb
# looks these up by process name "w11_<op_i>_<op_j>" (getYieldScale_ in
# AnomalousCouplingMorphing_comb.py) to build the mixed SM+Lin_i+Lin_j+Quad_i+Quad_j+2*M_ij term.
for _kk, (_op_i, _op_j) in enumerate(itertools.combinations(OPERATORS, 2)):
    _idx = 55 + _kk
    subsamples_eft[f"w11_{_op_i}_{_op_j}"] = (all_mask, f"{rwgt}[:, {_idx}]")
...
# -- EFT signal (DY SMEFTsim LO propcorr, 7 mll bins, 406 subsamples each) --
# EFT: 406 → runner filters events to those with exactly 406 LHEReweightingWeights.
# Unlike config_propcorr_v1.py (which only extracted the first 55 - SM + w1 + wm1),
# subsamples_eft above now extracts all 406, including the 351 cross terms.
for b in MLL_BINS:
    name = f"DYSMEFTsim_LO_propcorr_mll_{b}"
    datasets[name] = {
        "files": name,
        "task_weight": 8,
        "read_form": "mc",
...
samples = {
    ...
    # EFT signal subsamples (morphing convention) -- propcorr production
    # noStat=True: zeroes bin variances in shapes.root → autoMCStats does not
    # apply to EFT templates (bin errors are correlated across sm/w1/wm1/w11, not independent Poisson)
    "sm": {"samples": [f"DYSMEFTsim_LO_propcorr_mll_{b}_sm" for b in MLL_BINS], "noStat": True},
}
for _op in OPERATORS:
    samples[f"w1_{_op}"]  = {"samples": [f"DYSMEFTsim_LO_propcorr_mll_{b}_w1_{_op}"  for b in MLL_BINS], "noStat": True}
    samples[f"wm1_{_op}"] = {"samples": [f"DYSMEFTsim_LO_propcorr_mll_{b}_wm1_{_op}" for b in MLL_BINS], "noStat": True}
for _op_i, _op_j in itertools.combinations(OPERATORS, 2):
    samples[f"w11_{_op_i}_{_op_j}"] = {
        "samples": [f"DYSMEFTsim_LO_propcorr_mll_{b}_w11_{_op_i}_{_op_j}" for b in MLL_BINS],
        "noStat": True,
    }
...
for _i, (_op_i, _op_j) in enumerate(itertools.combinations(OPERATORS, 2)):
    colors[f"w11_{_op_i}_{_op_j}"] = cmap_petroff[_i % len(cmap_petroff)]
...
# EFT samples — theory systs apply to sm + all w1/wm1 operators + all w11 cross terms
eft_samples = (
    ["sm"]
