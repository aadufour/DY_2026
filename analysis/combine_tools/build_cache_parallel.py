#!/usr/bin/env python3
"""
build_cache_parallel.py
=======================
Parallelised version of build_cache_new.py.
Spawns one worker process per LHE mll bin — all 7 files are read simultaneously.

Each worker saves its result to a per-file checkpoint pkl.
If a checkpoint already exists for a file it is skipped, so interrupted
runs resume cheaply.

Typical speedup: ~6-7x vs sequential (wall-clock limited by the heaviest bin).

Usage:
    python3 build_cache_parallel.py
    python3 build_cache_parallel.py --eta-max 2.4 --pt-lead 25 --pt-sub 10
    python3 build_cache_parallel.py --nodoubles --nevents 5000
    python3 build_cache_parallel.py --workers 4   # limit parallelism
"""
...
CACHE_DIR       = "/grid_mnt/data__data.polcms/cms/adufour/LHE/SYST_slc7/CACHE"
CACHE_FILE      = os.path.join(CACHE_DIR, "lhe_cache_parallel.pkl")
...
def process_file(lhe_file):
    """
    Read one LHE file, apply cuts, accumulate arrays.
    Returns a dict of numpy arrays or loads from per-file checkpoint if present.
    """
    tag      = os.path.basename(os.path.dirname(lhe_file))
    ...
    if not os.path.exists(lhe_file):
        print(f"[{tag}] WARNING: file not found, skipping")
        return None
...
if __name__ == "__main__":
    ...
    print(f"Processing {len(LHE_FILES)} LHE files in parallel")
    if do_fiducial:
    ...
    n_workers = args.workers or len(LHE_FILES)
    with Pool(processes=n_workers) as pool:
        results = pool.map(process_file, LHE_FILES)

    # Filter failed files
    results = [r for r in results if r is not None]
    if not results:
        raise RuntimeError("No results — check LHE file paths.")
