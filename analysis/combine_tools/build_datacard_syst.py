#!/usr/bin/env python3
"""
build_datacard_syst.py

Like build_datacard.py but reads from the SYST_slc7 cache produced by
build_cache_syst.py. Adds --pdf-flavour to choose between the two PDF sets:

  --pdf-flavour 5   NNPDF31_nnlo_as_0118_mc_hessian_pdfas  (5-flavour, 103 members)
  --pdf-flavour 4   NNPDF31_nnlo_as_0118_nf_4_mc_hessian   (4-flavour, 101 members)

Everything else (operators, systematics, histogram building, datacard format)
is identical to build_datacard.py.

Usage:
    python3 build_datacard_syst.py --op cHDD --pdf-flavour 5
    python3 build_datacard_syst.py --op cHDD --C 0.5 --lumi 59740 --pdf-flavour 4
    python3 build_datacard_syst.py --op cHDD --unrolled --pdf-flavour 5
    python3 build_datacard_syst.py --op cHDD cHWB --C 1.0 --lumi 59740 --pdf-flavour 4
    python3 build_datacard_syst.py --all_op --lumi 59740 --pdf-flavour 5
"""
...
parser.add_argument("--pdf-flavour",  type=int, choices=[4, 5], required=True,
                    help="PDF set: 5 = NNPDF31_nnlo_as_0118_mc_hessian_pdfas (325300, 103 members); "
                         "4 = NNPDF31_nnlo_as_0118_nf_4_mc_hessian (325500, 101 members)")
args = parser.parse_args()

LUMI        = args.lumi
PDF_FLAVOUR = args.pdf_flavour
PDF_KEY     = "pdf_325300" if PDF_FLAVOUR == 5 else "pdf_325500"
PDF_LABEL   = "NNPDF31_nnlo_as_0118_mc_hessian_pdfas (325300)" if PDF_FLAVOUR == 5 \
              else "NNPDF31_nnlo_as_0118_nf_4_mc_hessian (325500)"

# --------------- Load cache ----------------------------------------

if not os.path.exists(args.cache):
    raise FileNotFoundError(f"Cache not found: {args.cache}\nRun build_cache_syst.py first.")
...
if has_pdf:
    ...
    print(f"  PDF ({PDF_KEY})  : not found in cache — will be skipped")
print()
...
# ---- Build Up/Down shape variants for all processes ----------------
