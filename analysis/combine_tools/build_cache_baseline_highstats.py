#!/usr/bin/env python3
"""
build_cache_baseline_highstats.py
==================================
Variant of build_cache_propcorr.py for the baseline (non-propagator-corrected)
sample, where most bins still have a single pre-merged unweighted_events.lhe
(the original LHE/SYST_slc7/ convention) but one or more bins additionally
have a follow-up high-stats batch of many separate files (from parallel
condor jobs, same layout as the propcorr production). Both sources are
combined per bin automatically -- no file copying needed.

Parallelism and checkpointing are per file, same as build_cache_propcorr.py:
every LHE file across every bin is its own task, so a bin with many extra
files spreads across all requested workers instead of hogging one core.

Usage:
    python3 build_cache_baseline_highstats.py
    python3 build_cache_baseline_highstats.py --eta-max 2.4 --pt-lead 25 --pt-sub 10
    python3 build_cache_baseline_highstats.py --nodoubles --nevents 5000
    python3 build_cache_baseline_highstats.py --workers 10
"""
...
parser.add_argument('--workers', type=int, default=None,
                    help='Number of parallel workers (default: one per file)')
...
SYST_BASE      = "/grid_mnt/data__data.polcms/cms/adufour/LHE/SYST_slc7"
HIGHSTATS_BASE = "/grid_mnt/data__data.polcms/cms/adufour/LHE/baseline"


def _bin_files(lo, hi):
    """Original single merged file (if present) + any high-stats follow-up
    batch files (if that bin has one) -- combined transparently."""
    tag = f"DYSMEFTMll{lo}_{hi}"
    files = []

    original = f"{SYST_BASE}/{tag}/unweighted_events.lhe"
    if os.path.exists(original):
        files.append(original)

    extra_dir = f"{HIGHSTATS_BASE}/{tag}/lhe_output"
    if os.path.isdir(extra_dir):
        files += sorted(glob.glob(f"{extra_dir}/output_*.lhe"))

    return tag, files


# one (tag, [file list]) pair per bin
LHE_BINS = [_bin_files(lo, hi) for lo, hi in zip(MLL_BIN_EDGES[:-1], MLL_BIN_EDGES[1:])]
...
CACHE_DIR       = "/grid_mnt/data__data.polcms/cms/adufour/LHE/baseline/CACHE"
CACHE_FILE      = os.path.join(CACHE_DIR, "lhe_cache_baseline_highstats.pkl")
...
def process_one_file(task):
    ...
    Returns (tag, result_dict) or loads from a per-file checkpoint if present.
    """
    tag, lhe_file = task
...
if __name__ == "__main__":
    ...
    print(f"Processing {len(LHE_BINS)} mll bins")
    ...
    if not results:
        raise RuntimeError("No results — check SYST_BASE / HIGHSTATS_BASE paths.")
